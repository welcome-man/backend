$(function () {
    $("#jqGrid").jqGrid({
        ...
        colModel: [			
			...
			{ label: '采样值', name: 'sampleValue', width: 80 }, 			
			{ label: '示值误差', name: 'indicationError', width: 80 }, 			
...
var vm = new Vue({
	...
	methods: {
		...
		saveOrUpdate: function (event) {
			var url = vm.checkData.id == null ? "../checkdata/save" : "../checkdata/update";
			$.ajax({
				type: "POST",
			    url: url,
			    data: JSON.stringify(vm.checkData),
			    success: function(r){
			    	if(r.code === 0){
		...
		getInfo: function(id){
			$.get("../checkdata/info/"+id, function(r){
                vm.checkData = r.checkData;
            });
		},
